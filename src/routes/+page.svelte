<script lang="ts">
  ...
  const BENEFITS = [
    {
      icon: 'M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8l1 12h12L19 8',
      title: 'Free Delivery',
      desc: 'On orders above ₦50,000 across Nigeria.',
    },
    {
      icon: 'M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z',
      title: 'Authentic Products',
      desc: 'Every product is 100% genuine.',
    },
    {
...
<svelte:head>
  ...
  <script type="application/ld+json">{JSON.stringify({
    ...
    "@type": "WebSite",
    "name": "ShopHub",
    "url": "https://shophub.ng",
    "potentialAction": {
      "@type": "SearchAction",
      "target": "https://shophub.ng/catalog?q={search_term_string}",
      ...
      <div class="flex flex-wrap items-center gap-4 mb-10">
        {#each ['10,000+ customers', 'Same-day Lagos delivery', '100% authentic'] as badge}
          <div class="flex items-center gap-1.5 text-sm text-muted">
            <svg class="w-4 h-4 text-success flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
        ...
        {#if products[0]}
          ...
          <div class="absolute bottom-6 left-6 right-6 bg-surface/95 backdrop-blur-sm rounded-2xl p-4 shadow-card-hover">
            <div class="flex items-center justify-between">
              ...
              <button
                on:click={() => addToCart(products[0])}
                class="w-10 h-10 rounded-2xl bg-primary grid place-items-center text-white hover:bg-accent transition-colors"
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
  <div class="flex items-end justify-between mb-8">
    ...
    <a href="/catalog" class="nav-link flex items-center gap-1.5 font-medium">
      All products
      <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
    </a>
  </div>
  ...
  <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    {#each CATEGORIES_FEATURED as cat, i}
      <a
        href="/catalog?category={encodeURIComponent(cat.name)}"
        class="group relative overflow-hidden rounded-3xl aspect-[4/5] bg-primary/5"
...
<!-- ======================== PRODUCT SECTION ======================== -->
{#snippet productCard(product: any, delay: number = 0)}
  <div class="group relative flex flex-col" style="animation-delay:{delay}ms">
    <!-- Wishlist -->
    <button
      on:click={() => toggleWishlist(product)}
      class="absolute top-3 right-3 z-10 w-8 h-8 rounded-full bg-surface/90 backdrop-blur-sm grid place-items-center shadow-card opacity-0 group-hover:opacity-100 transition-all duration-200 hover:bg-surface"
      aria-label="Toggle wishlist"
    >
      <svg class="w-4 h-4 transition-colors" class:text-danger={isWishlisted(product.id)} class:text-muted={!isWishlisted(product.id)} fill={isWishlisted(product.id) ? 'currentColor' : 'none'} stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    <a href="/product/{product.id}" class="block">
      ...
      <div class="px-1">
        ...
        <div class="flex items-center gap-1.5 mb-2">
          <div class="flex">
            {#each [1,2,3,4,5] as s}
              <svg class="w-3.5 h-3.5 {s<=4?'text-accent':'text-primary/15'}" viewBox="0 0 20 20" fill="currentColor">
    ...
    <button
      ...
      class="mt-3 w-full h-10 rounded-2xl bg-primary text-white text-sm font-semibold hover:bg-accent active:scale-[0.98] transition-all duration-150 opacity-0 group-hover:opacity-100"
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
  <div class="flex items-end justify-between mb-8">
    ...
    <a href="/catalog" class="nav-link flex items-center gap-1.5 font-medium">
      View all <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
    </a>
  </div>
  ...
  {:else}
    <div class="grid grid-cols-2 md:grid-cols-4 gap-5">
      {#each trending as product, i}
        {@render productCard(product, i * 60)}
      {/each}
    </div>
  {/if}
</section>
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
  <div class="grid grid-cols-2 lg:grid-cols-4 gap-6">
    {#each BENEFITS as b, i}
      <div class="text-center p-6 rounded-3xl bg-surface shadow-card border border-primary/5 animate-fade-up" style="animation-delay:{i*80}ms">
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
  <div class="bg-primary rounded-4xl px-8 py-14 md:px-16 text-center relative overflow-hidden">
    ...
    <div class="relative">
      ...
      <h2 class="text-[2rem] md:text-section font-bold tracking-tight text-white mb-4 text-balance">
      ...
      {:else}
        <form on:submit={handleNewsletter} class="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
          <input
            ...
            class="flex-1 h-12 px-5 rounded-2xl bg-white/10 border border-white/20 text-white placeholder:text-white/40 text-sm focus:outline-none focus:ring-2 focus:ring-accent/50 transition"
          />
          <button type="submit" class="btn-accent h-12 px-7 flex-shrink-0">
            Subscribe
          </button>
        </form>
      {/if}
